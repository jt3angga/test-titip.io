...
export function Map() {
  ...
  const { data, isError, error } = useGetCoordinatesQuery(
  ...
  useEffect(() => {
    if (selectedMap.selectedPorts?.length === 1) {
      ...
      const targetFlyTo = {
        center: [coord.coordinates[0], coord.coordinates[1]],
        zoom: 11,
        speed: 1,
        bearing: 0,
        pitch: 0,
  ...
  useEffect(() => {
    const coordinates =
      selectedMap.dataSource?.features[0].geometry.coordinates;
  ...
  return (
    ...
    >
      ...
      {selectedMap.dataSource ? (
        <Source id="route" type="geojson" data={selectedMap.dataSource}>
          <Layer {...lineLayer} />
        </Source>
      ) : null}
    </ReactMapGl>
